...
export default function CreateScreens() {
  const { loadStrategyById } = useExecuteQuery();
  ...
  /** Prefetched detail data for SearchOutput */
  ...
  useEffect(() => {
    if (location.state?.initialQuery) {
      ...
      handleSearchButtonClick(location.state.initialQuery, location.state.sessionId);
    }
  }, [location.state]);

  // If navigated with a strategyId (e.g., from Popular table), load the full data by ID
  useEffect(() => {
    const id = location.state?.strategyId;
    if (!id) return;
    (async () => {
      try {
        setIsLoading(true);
        const data = await loadStrategyById(id);
        setPrefetchedData(data);
        setIsQueryHidden(true);
        setShowOutput(true);
      } catch (_e) {
        // leave UI as-is; errors are handled in the hook
      } finally {
        setIsLoading(false);
      }
    })();
