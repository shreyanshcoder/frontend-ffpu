...
const SearchOutput = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    // Prefer explicit initialData from parent; fall back to shared hook state
    const initial = props.initialData || responseData;
    if (initial && initial.output) {
      console.log('Setting portfolio data:', initial.output); // Debug log
      setPortfolioData(initial.output);
    }
