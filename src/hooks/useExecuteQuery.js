...
const useExecuteQuery = () => {
  ...
  /**
   * Loads a saved strategy by its identifier and normalizes the response
   * so that SearchOutput can render it (same shape as executeQuery)
   *
   * @param {string} strategyId - Strategy identifier (strat_uuid)
   * @returns {Promise<Object>} Normalized response { status, output, strategy_id }
   */
  const loadStrategyById = async (strategyId) => {
    ...
    try {
      ...
      const errorMessage = axios.isAxiosError(err)
        ...
        : err?.message || 'Failed to load strategy';
      setError(errorMessage);
      sharedError = errorMessage;
      console.error('Error loading strategy by id:', errorMessage);
      throw err;
    } finally {
      setLoading(false);
      sharedLoading = false;
  ...
  return { executeQuery, saveStrategy, loadStrategyById, loading, error, responseData };
};
